...
const registerRenderEvents = (scene: Scene, events: Events) => {
    ...
    events.function('render.image', async (imageSettings: ImageSettings) => {
        ...
        try {
            ...
            const filename = `${removeExtension(selected?.name ?? 'PointForge')}-image.png`;
    ...
    events.function('render.video', (videoSettings: VideoSettings, fileStream: FileSystemWritableFileStream) => {
        const renderImpl = async () => {
            ...
            try {
                ...
                if (!cancelled && !fileStream) {
                    ...
                    downloadFile((output.target as BufferTarget).buffer, `${removeExtension(currentSplats[0]?.name ?? 'pointforge')}.${fileExtension}`);
                }
        ...
        // codec reclamation.
        if (navigator.locks) {
            return navigator.locks.request('pointforge-video-render', renderImpl);
        }
        return renderImpl();
    });
};

export type { ImageSettings, VideoSettings };
